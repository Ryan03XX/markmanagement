        ...
        <div class="main-content">
            ...
            <main class="dashboard">
                ...
                <div class="search-section">
                    <input type="text" v-model="searchId" placeholder="Enter Student Matric Number..." />
                ...
                <div v-if="courses.length > 0" class="results-section">
                    ...
                    <div v-for="course in courses" :key="course.id" class="course-card">
                        ...
                        <div v-if="course.result">
                            <p><strong>Final Exam Mark:</strong> {{ course.result.final_exam_mark ?? 'N/A' }}</p>
                            <p><strong>Final Mark:</strong> {{ course.result.final_mark ?? 'N/A' }}</p>
                            <div class="component-marks">
                                <p><strong>Component Marks:</strong></p>
                                <ul>
                                    <li v-for="comp in course.result.component_results" :key="comp.component_name">
                ...
                <div v-if="searched && courses.length === 0">
                    <p style="text-align: center; color: red;">No enrolled courses found for this student.</p>
                </div>
            </main>
        </div>
    </div>
...
export default {
    ...
    methods: {
        ...
        async searchStudent() {
            ...
            try {
                // ✅ 使用正确的 matric_no 路由
                const res = await fetch(`http://localhost:8080/api/student/matric/${this.searchId}/enrolled-courses`)
                ...
                if (data.success) {
                    ...
                    for (let course of this.courses) {
                        // ✅ 使用 by-matric-no 路由
                        const resultRes = await fetch(`http://localhost:8080/api/student/by-matric-no/${this.searchId}/course/${course.id}/result`)
                        const resultData = await resultRes.json()

                        if (resultData.success) {
                            // 兼容旧代码字段结构
                            course.result = {
                                final_exam_mark: resultData.final_result?.final_exam_mark ?? null,
